package com.leixia.pubsub.message.listener;
...
public class DelayedKafkaMessageListenerAdaptor<K, V> extends
    AbstractDelegatingMessageListenerAdapter<MessageListener<K, V>>
  ...
  public DelayedKafkaMessageListenerAdaptor(MessageListener<K, V> delegate,
      KafkaConsumerBackoffManager kafkaConsumerBackoffManager, String listenerId) {
    super(delegate);
    Objects.requireNonNull(kafkaConsumerBackoffManager,
        "kafkaConsumerBackoffManager cannot be null");
  ...
  public void onMessage(ConsumerRecord<K, V> consumerRecord, Acknowledgment acknowledgment,
      Consumer<?, ?> consumer) throws KafkaBackoffException {
    this.kafkaConsumerBackoffManager.backOffIfNecessary(
        createContext(consumerRecord, consumerRecord.timestamp() +
  ...
  public void setDefaultDelay(Duration delay) {
    Objects.requireNonNull(delay, "Delay cannot be null");
    logger.debug(
        () -> String.format("Setting %s seconds delay for listener id %s", delay, this.listenerId));
    defaultDelay = delay;
  }

  private void invokeDelegateOnMessage(ConsumerRecord<K, V> consumerRecord,
  ...
  private KafkaConsumerBackoffManager.Context createContext(ConsumerRecord<K, V> data,
      long nextExecutionTimestamp, Consumer<?, ?> consumer) {
